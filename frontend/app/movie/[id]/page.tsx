...
export default function MovieDetail() {
    ...
    const toggleLike = async () => {
        if (!user || !movie) return;
        const userRef = doc(db, "users", user.uid);
        const movieData = { id: movie.id, title: movie.title, poster_path: movie.poster_path, vote_average: movie.vote_average };
    ...
    const updateRating = async (val: number) => {
        if (!user || !movie) return;
        const userRef = doc(db, "users", user.uid);
        const movieData = { id: movie.id, title: movie.title, poster_path: movie.poster_path, vote_average: movie.vote_average, userRating: val };
    ...
    const toggleCustomList = async (listId: string, listName: string, currentlyInList: boolean) => {
        if (!user || !movie) return;
        const userRef = doc(db, "users", user.uid);
        const movieData = { id: movie.id, title: movie.title || "Unknown", poster_path: movie.poster_path, vote_average: movie.vote_average, media_type: "movie" };
                    ...
                    <div className="flex justify-between items-start gap-6">
                        <div>
                            ...
                            <div className="flex items-center gap-4 text-sm font-semibold uppercase tracking-widest text-[#678] mb-8 mt-4">
                                ...
                                <span>Directed by <span className="text-brand-light font-bold hover:text-brand-blue cursor-pointer transition-colors border-b border-transparent hover:border-brand-blue">{director}</span></span>
                            </div>
                        </div>
